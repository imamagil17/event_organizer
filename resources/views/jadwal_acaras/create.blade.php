<x-default-layout title="Tambah Jadwal Acara" section_title="Tambah Jadwal Acara">
    <div class="container mt-5 pb-5">
        <div class="card shadow-sm border-0 rounded-lg">
            <div class="card-header bg-primary text-white">
                <h4 class="m-0">Tambah Jadwal Acara</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('jadwal_acaras.store') }}" method="POST">
                    @csrf
                        ...
                        <select name="acara_id" id="acara_id"
                            ...
                            @foreach ($acaras as $acara)
                                <option value="{{ $acara->id }}"
                                    {{ old('acara_id') == $acara->id ? 'selected' : '' }}>
                                    {{ $acara->judul }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="text" name="judul_kegiatan" id="judul_kegiatan"
                            ...
                            value="{{ old('judul_kegiatan') }}" placeholder="Masukkan judul kegiatan" required>
